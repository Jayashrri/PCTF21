...
class Point :
	...
	def __add__(self, P) :
		try :
			...
			if (self.x, self.y) == (P.x, P.y) : return P.double()
...
class Challenge :
	...
	def __init__(self, field_order, params, generator, public_key) :
	...
	def __call__(self) :
		...
		try :
			...
			answer = int(input(f'\n  SECRET KEY (hex) : '), 16) #23fcf62e4ca5319696cfcae39d75860c
			assert self.P.xy() == self.G.multiply(answer).xy()
			flag = open(self.flag_file, 'r').read().strip()
			print(f"\n  Here is your flag : {flag}\n")
		except AssertionError :
			print("\n  OOPS! Keep trying ... never gonna give you up ^_^\n")
		return
...
def main() :
	...
	challenge = Challenge(M, params, G, P)
	return challenge()
